...
from .trainer_utils import TimeStrategy
...
class DefaultFlowCallback(TrainerCallback):
    ...
    def on_step_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        if (
            args.logging_strategy == TimeStrategy.STEPS
        ...
        # Evaluate
        if args.evaluation_strategy == TimeStrategy.STEPS and state.global_step % args.eval_steps == 0:
        ...
        if (
            ...
            and args.save_strategy == TimeStrategy.STEPS
    ...
    def on_epoch_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        # Log
        if args.logging_strategy == TimeStrategy.EPOCH:
            control.should_log = True

        # Evaluate
        if args.evaluation_strategy == TimeStrategy.EPOCH:
            control.should_evaluate = True
            if args.load_best_model_at_end:
                control.should_save = True

        # Save
        if args.save_strategy == TimeStrategy.EPOCH:
            control.should_save = True
...
class EarlyStoppingCallback(TrainerCallback):
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        assert (
            args.evaluation_strategy != TimeStrategy.NO
        ), "EarlyStoppingCallback requires TimeStrategy of steps or epoch"
